    ...
    <el-card>
      ...
      <el-row>
        <el-table :data="orderList" border>
          ...
          <el-table-column prop="order_price" label="订单价格" width="220px"></el-table-column>
      ...
      <!--      分页区-->
      <el-row>
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="queryInfo.pagenum" :page-sizes="[5, 9, 11, 15]" :page-size="queryInfo.pagesize" layout="total, sizes, prev, pager, next, jumper" :total="total"> </el-pagination>
      </el-row>
    </el-card>
    <!--    修改地址的对话框-->
...
export default {
  ...
  methods: {
    // 获取订单列表数据
    async getOrderList() {
      const { data: res } = await orderListAPI(this.queryInfo)
